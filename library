...
local themes = {
    preset = {
        ...
        ["inline"] = hex("#2D1A20"),
        ["accent"] = hex("#E05B7F"),
        ["high_contrast"] = hex("#160A0F"),
        ["low_contrast"] = hex("#1E1015"),
        ["text"] = hex("#D4A4B4"),
...
-- elements 
    ...
    function library:window(properties)
        ...
        -- main window
            local win_size = (properties and properties.size) or dim2(0, 604, 0, 628)
            local main_window = library:panel({
                name = properties and properties.name or "amethyst | ", 
                size = win_size,
                position = dim2(0, (camera.ViewportSize.X / 2) - (win_size.X.Offset / 2) - 96, 0, (camera.ViewportSize.Y / 2) - (win_size.Y.Offset / 2) - 12),
        ...
        -- theming 
            ...
            local watermark = library:watermark({default = os.date('amethyst |  - %b %d %Y - %H:%M:%S')})  

            task.spawn(function()
                local game_label = library.supported_games[game.PlaceId] or "Universal"
                while task.wait(1) do 
                    watermark.change_text(os.date('amethyst - ' .. game_label .. ' - %b %d %Y - %H:%M:%S'))
                end 
            end) 
            ...
            local theme_presets = {
                ...
                ["Atlanta2"] = {
                    accent        = rgb(154, 127, 172),
                    outline       = rgb(53, 53, 67),
                    inline        = rgb(42, 42, 56),
                    high_contrast = rgb(36, 36, 47),
                    low_contrast  = rgb(42, 42, 56),
                    text          = rgb(200, 200, 200),
                    text_outline  = rgb(0, 0, 0),
                    glow          = rgb(154, 127, 172),
                },
                ["Amethyst"] = {
                    accent        = hex("#9B6FD4"),
                    outline       = hex("#0A0A0A"),
                    inline        = hex("#2A1F3D"),
                    high_contrast = hex("#120D1E"),
                    low_contrast  = hex("#1A1228"),
                    text          = hex("#C4B4D4"),
                    text_outline  = rgb(0, 0, 0),
                    glow          = hex("#9B6FD4"),
                },
                ...
                ["Emerald"] = {
                    ...
                    outline       = hex("#0A0A0A"),
                    inline        = hex("#1A2D25"),
                    high_contrast = hex("#0D1610"),
                    low_contrast  = hex("#121E18"),
                    text          = hex("#A4D4BC"),
                    text_outline  = rgb(0, 0, 0),
                    glow          = hex("#4CAF85"),
                },
                ["Gold"] = {
                    ...
                    outline       = hex("#0A0A0A"),
                    inline        = hex("#2D2515"),
                    high_contrast = hex("#160F05"),
                    low_contrast  = hex("#1E180A"),
                    text          = hex("#D4C484"),
                    text_outline  = rgb(0, 0, 0),
                    glow          = hex("#D4A43C"),
                },
                ["Crimson"] = {
                    ...
                    outline       = hex("#0A0A0A"),
                    inline        = hex("#2D1515"),
                    high_contrast = hex("#160808"),
                    low_contrast  = hex("#1E0D0D"),
                    text          = hex("#D4A4A4"),
                    text_outline  = rgb(0, 0, 0),
                    glow          = hex("#DC3545"),
                },
                ["Cyan"] = {
                    ...
                    outline       = hex("#0A0A0A"),
                    inline        = hex("#1A2A2D"),
                    high_contrast = hex("#0D1416"),
                    low_contrast  = hex("#121C1E"),
                    text          = hex("#A4CCD4"),
                    text_outline  = rgb(0, 0, 0),
                    glow          = hex("#38BCD4"),
                },
            ...
            local function apply_theme_preset(name)
                ...
                if not library.config_flags["Accent"] then return end
                local function set(flag, color)
                    if library.config_flags[flag] then
                        library.config_flags[flag](color, 1)
                    end
                end
                set("Accent",        p.accent)
                set("low_contrast",  p.low_contrast)
                set("high_contrast", p.high_contrast)
                set("Inline",        p.inline)
                set("Outline",       p.outline)
                set("Main",          p.text)
                set("Glow",          p.glow)
                library:update_theme("text_outline", p.text_outline)
                library:update_theme("contrast", rgbseq{
                    rgbkey(0, p.low_contrast),
                    rgbkey(1, p.high_contrast)
                })
            end

            local preset_names = {}
            for k in next, theme_presets do preset_names[#preset_names + 1] = k end
            ...
            :keybind({callback = window.set_menu_visibility, key = Enum.KeyCode.Delete})
        ...
        -- cfg holder
            ...
            local section = column:section({name = "Options"})
                ...
                section:button({name = "Unload Menu", callback = function()
                    ...
                    if library.on_unload then pcall(library.on_unload) end
    ...
    function library:watermark(options) 
        ...
        local text = library:create("TextLabel", {
            ...
            TextColor3 = themes.preset.text,
            BorderColor3 = rgb(0, 0, 0),
            Text = "  drain.lol | Aug 29 2024 | 07:29:00  ",
            Size = dim2(0, 0, 1, 0),
            BackgroundTransparency = 1,
            Position = dim2(0, -1, 0, 1),
            BorderSizePixel = 0,
    ...
    function library:esp_preview(properties)
        ...
        local amethyst = getgenv and getgenv().amethyst
        local names_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Names"]
        local boxes_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Boxes"]
        local dist_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Distance"]
        local weapon_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Weapon"]
        local function esp_color(fc, key)
            if not fc or not fc["Colors"] then return rgb(255, 255, 255) end
            return fc["Colors"][key or "Neutral"] or fc["Colors"]["Neutral"] or rgb(255, 255, 255)
        end
        ...
        local objects = cfg.objects; do 
            ...
            objects[ "name" ] = library:create( "TextLabel" , {
                FontFace = library.font;
                Parent = library.cache;
                TextColor3 = esp_color(names_cfg, "Neutral");
                BorderColor3 = rgb(0, 0, 0);
            ...
            -- Corner Boxes
                ...
                library:create( "Frame" , {
                    ...
                    Size = dim2(1, -2, 1, -2);
                    BorderSizePixel = 0;
                    BackgroundColor3 = esp_color(boxes_cfg, "Neutral")
                });
            ...
            -- Distance esp
                objects[ "distance" ] = library:create( "TextLabel" , {
                    FontFace = library.font;
                    TextColor3 = esp_color(dist_cfg, "Neutral");
                    BorderColor3 = rgb(0, 0, 0);
                    Text = "127st";
                    Parent = library.cache;
                    TextStrokeTransparency = 0;
            ...
            -- Weapon esp
                objects[ "weapon" ] = library:create( "TextLabel" , {
                    FontFace = library.font;
                    TextColor3 = esp_color(weapon_cfg, "Neutral");
                    BorderColor3 = rgb(0, 0, 0);
                    Text = "[ Weapon ]";
                    Parent = library.cache;
                    TextStrokeTransparency = 0;
        ...
        cfg.change_health = function()
            ...
            local amethyst = getgenv and getgenv().amethyst
            local health_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Health"]
            local low_c = (health_cfg and health_cfg["Low"]) or (flags["Health_Low"] and flags["Health_Low"].Color) or rgb(255, 0, 0)
            local high_c = (health_cfg and health_cfg["High"]) or (flags["Health_High"] and flags["Health_High"].Color) or rgb(0, 255, 0)
            local color = low_c:Lerp(high_c, multiplier)
        ...
        function cfg.refresh_elements( )                                
            ...
            local amethyst = getgenv and getgenv().amethyst
            local boxes_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Boxes"]
            local boxes_enabled = boxes_cfg and boxes_cfg["Enabled"]
            local names_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Names"]
            local health_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Health"]
            local dist_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Distance"]
            local weapon_cfg = amethyst and amethyst["Esp"] and amethyst["Esp"]["Weapon"]
            local show_names = names_cfg and names_cfg["Enabled"]
            local show_health = health_cfg and health_cfg["Enabled"]
            local show_dist = dist_cfg and dist_cfg["Enabled"]
            local show_weapon = weapon_cfg and weapon_cfg["Enabled"]
            local box_type = (boxes_cfg and boxes_cfg["Type"]) or (flags["Box_Type"]) or "Corner"

            local preview_priority = amethyst and amethyst["Esp"] and amethyst["Esp"]["PreviewPriority"]
            local name_color = (names_cfg and names_cfg["Colors"] and (names_cfg["Colors"][preview_priority] or names_cfg["Colors"]["Neutral"])) or rgb(255, 255, 255)
            local dist_color = (dist_cfg and dist_cfg["Colors"] and (dist_cfg["Colors"][preview_priority] or dist_cfg["Colors"]["Neutral"])) or rgb(255, 255, 255)
            local weapon_color = (weapon_cfg and weapon_cfg["Colors"] and (weapon_cfg["Colors"][preview_priority] or weapon_cfg["Colors"]["Neutral"])) or rgb(255, 255, 255)
            local box_color = (boxes_cfg and boxes_cfg["Colors"] and (boxes_cfg["Colors"][preview_priority] or boxes_cfg["Colors"]["Neutral"])) or rgb(255, 255, 255)
            local filled_colors = boxes_cfg and boxes_cfg["Filled"] and boxes_cfg["Filled"]["Colors"]
            local fill_color = (filled_colors and (filled_colors[preview_priority] or filled_colors["Neutral"])) or rgb(255, 255, 255)
            objects["name"].Parent = show_names and objects["holder"] or library.cache
            objects["name"].TextColor3 = name_color
            objects["healthbar_holder"].Parent = show_health and objects["holder"] or library.cache
            objects["distance"].Parent = show_dist and objects["holder"] or library.cache
            objects["distance"].TextColor3 = dist_color
            objects["weapon"].Parent = show_weapon and objects["holder"] or library.cache
            objects["weapon"].TextColor3 = weapon_color
            ...
            if boxes_enabled then
                if box_type == "Corner" then
                    ...
                    objects[ "box_fill" ].Parent = library.cache
                    for _, corner in objects[ "corners" ]:GetChildren() do
                        if corner:FindFirstChild("Frame") then
                    ...
                    local filled = boxes_cfg and boxes_cfg["Filled"]
                    if filled and filled["Enabled"] then
                        objects[ "box_fill" ].Parent = objects[ "box_handler" ]
                        objects[ "box_fill" ].BackgroundColor3 = fill_color
                        local alpha = filled["Alpha"] or (filled["Transparency"] and (1 - filled["Transparency"]) or 0.7)
                        objects[ "box_fill" ].BackgroundTransparency = 1 - alpha
                    else
                        objects[ "box_fill" ].Parent = library.cache
                    end
                end 
    ...
    function library:tab(options)	
        ...
        function cfg.open_tab()
            if library.current_tab and library.current_tab[1] ~= background then 
                ...
                local ug = button:FindFirstChildOfClass("UIGradient")
                if ug then ug.Rotation = 90 end
                local tl = button:FindFirstChildOfClass("TextLabel")
                if tl then tl.TextColor3 = themes.preset.text end
            ...
            if ug then ug.Rotation = -90 end
            local tl = button:FindFirstChildOfClass("TextLabel")
            if tl then tl.TextColor3 = themes.preset.accent end 
    ...
    function library:slider(options)
        ...
        -- instances 
            ...
            if cfg.name then 
                ...
                if cfg.risky then library:apply_theme(TEXT_LABEL, "risky", "TextColor3") end
    ...
    function library:toggle(options)
        ...
        -- instances
            ...
            if cfg.risky then library:apply_theme(text, "risky", "TextColor3") end
    ...
    function library:colorpicker(options)
        ...
        local parent
        if not self.right_holder and cfg.flag then
            ...
            library:create("UIPadding", { Parent = row })
            local left_components = library:create("Frame", {
                Parent = row,
                Name = "left_components",
                BackgroundTransparency = 1,
            ...
            local lbl = left_components:FindFirstChild("text")
            if lbl then library:create("UIStroke", { Parent = lbl, LineJoinMode = Enum.LineJoinMode.Miter }) end
            library:create("UIListLayout", { Parent = left_components, Padding = dim(0, 5), Name = "_", FillDirection = Enum.FillDirection.Horizontal })
            cfg.set_element_visible = function(bool) row.Visible = bool end
            library.visible_flags[cfg.flag] = cfg.set_element_visible
            cfg.set_element_visible(cfg.visible)
            parent = right_components
        else
            parent = self.right_holder
        end
        ...
        local colorpicker_button = library:create("TextButton", {
            ...
            if cfg.conditional and cfg.flag then
                cfg.set_element_visible = function(bool) colorpicker_button.Visible = bool end
                library.visible_flags[cfg.flag] = cfg.set_element_visible
                cfg.set_element_visible(cfg.visible)
            end
        ...
        -- colorpicker instances
            ...
            section:button({name = "Copy", callback = function()
                ...
                library.copied_rainbow = RainbowToggle.enabled
            end})
            section:button({name = "Paste", callback = function()
                if library.copied_flag then
                    RainbowToggle.set(library.copied_rainbow or false)
    ...
    function library:dropdown(options)
        ...
        local cfg = {
            ...
            visible = options.visible or true,
            risky = options.risky or false,
            risky_values = options.risky_values or {},
        ...
        -- dropdown elements
            ...
            if cfg.name then 
                ...
                if cfg.risky then library:apply_theme(main_text, "risky", "TextColor3") end
        ...
        function cfg.set(value) 
            ...
            local display_val = selected[1]
            local is_risky = display_val and find(cfg.risky_values, display_val)
            text.TextColor3 = is_risky and themes.preset.risky or themes.preset.text
    ...
    function library:label(options)
        ...
        function cfg.set(text, copy_value) 
            TextLabel.Text = text 
            if options.copy_button and copy_value ~= nil then
                cfg.copy_value = copy_value
            end
        end

        if options.copy_button then
            cfg.copy_value = ""
            local copy_btn = library:create("TextButton", {
                Parent = right_components,
                Name = "",
                FontFace = library.font,
                Text = "Copy",
            ...
            library:apply_theme(copy_btn, "inline", "BackgroundColor3")
            library:apply_theme(copy_btn, "text", "TextColor3")
            copy_btn.MouseButton1Click:Connect(function()
                if cfg.copy_value and cfg.copy_value ~= "" then
                    setclipboard(cfg.copy_value)
                end
            end)
        end

        if cfg.flag then
            cfg.set_element_visible = function(bool) dropdown.Visible = bool end
            library.visible_flags[cfg.flag] = cfg.set_element_visible
            cfg.set_element_visible(cfg.visible)
        end
    ...
    function library:playerlist(options) 
        ...
        function cfg.create_player(player) 
            ...
            library.playerlist_data[username] = {}
            local path = library.playerlist_data[username]
            ...
            local priority_text = library:create("TextLabel", {
                ...
                Name = "",
                FontFace = library.font,
                TextColor3 = username ~= lp.Name and themes.preset.text or rgb(0, 0, 255),
                BorderColor3 = rgb(0, 0, 0),
                Text = username ~= lp.Name and "Neutral" or "LocalPlayer",
                BackgroundTransparency = 1,
                TextXAlignment = Enum.TextXAlignment.Left,
                BorderSizePixel = 0,
                AutomaticSize = Enum.AutomaticSize.Y,
            ...
            local saved_priority = library.priority_saves[player.UserId]
            if saved_priority then
                path.priority = saved_priority
                priority_text.Text = saved_priority
                priority_text.TextColor3 = patterns[saved_priority]
            elseif username ~= lp.Name then
                task.spawn(function()
                    local ok, is_friend = pcall(function() return lp:IsFriendsWith(player.UserId) end)
                    if ok and is_friend and not library.priority_saves[player.UserId] then
                        path.priority = "Friendly"
                        priority_text.Text = "Friendly"
                        priority_text.TextColor3 = patterns["Friendly"]
                        library.priority_saves[player.UserId] = "Friendly"
                    end
                end)
            end

            TextButton.MouseButton1Click:Connect(function()
                if username == lp.Name then 
                    return 
                end 
                ...
                library.selected_player = username
                library.config_flags["PLAYERLIST_DROPDOWN"](path.priority_text.Text)

                if cfg.labels.name then 
                    cfg.labels.name.set("User: " .. username, username)
                    cfg.labels.display.set("DisplayName: " .. players[username].DisplayName, tostring(players[username].DisplayName))
                    cfg.labels.uid.set("User Id: " .. players[username].UserId, tostring(players[username].UserId))
                end
            end)
        ...
        function cfg.search(text)
            ...
            for _, player in next, players:GetPlayers() do 
                ...
                if path then 
                    local match = string.lower(name):find(lower_text, 1, true)
                        or string.lower(player.DisplayName):find(lower_text, 1, true)
                    path.instance.Visible = match ~= nil
                    path.line.Visible = match ~= nil
                end 
            end 
        end 
        ...
        local friendly_file = library.directory .. "/friendlies.json"

        save_friendlies = function()
            local list = {}
            for uid, pri in pairs(library.priority_saves) do
                if pri == "Friendly" then
                    list[#list + 1] = tostring(uid)
                end
            end
            pcall(function()
                writefile(friendly_file, http:JSONEncode(list))
            end)
        end

        pcall(function()
            local content = readfile(friendly_file)
            local list = http:JSONDecode(content)
            if type(list) == "table" then
                for _, uid_str in ipairs(list) do
                    local uid = tonumber(uid_str) or uid_str
                    library.priority_saves[uid] = "Friendly"
                end
            end
        end)
        ...
        function library.prioritize(text) 
            ...
            local player_obj = players:FindFirstChild(library.selected_player)
            if player_obj then
                library.priority_saves[player_obj.UserId] = text
            end
            save_friendlies()
        end 
        ...
        for _, player in players:GetPlayers() do 
            local player_object = cfg.create_player(player)
...
library.lp            = lp
library.tween_service = tween_service
library.vec2          = vec2
library.vec3          = vec3
library.rgb           = rgb
library.hex           = hex
